...
class LinearLayer(Layer):
    ...
    def backward(self, previous_partial_gradient: np.ndarray) -> np.ndarray:
        ...
        grad = np.dot(previous_partial_gradient, self.weight.data.T)
        dw = np.dot(self.data.T, previous_partial_gradient)
        db = np.sum(previous_partial_gradient, axis=0, keepdims=True)
        self.weight.grad = dw
        self.bias.grad = db
        return grad
